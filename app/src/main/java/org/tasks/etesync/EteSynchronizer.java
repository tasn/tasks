...
public class EteSynchronizer {
  ...
  public void sync(CaldavAccount account) {
    ...
    if (isNullOrEmpty(account.getEncryptionPassword())) {
  ...
  private void synchronize(CaldavAccount account)
      throws IOException, DavException, KeyManagementException, NoSuchAlgorithmException, IntegrityException, VersionTooNewException, Exceptions.HttpException {
    EteSyncClient client = this.client.forAccount(account);
    List<Response> resources = client.getCalendars();
    ...
    for (Response resource : resources) {
      ...
      sync(calendar, resource, client.getHttpClient());
    }
    setError(account, "");
  }
