...
public abstract class BaseCaldavAccountSettingsActivity extends ThemedInjectingAppCompatActivity
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState == null) {
      if (caldavAccount != null) {
        ...
        if (!isEmpty(caldavAccount.getEncryptionPassword())) {
  ...
  private boolean encryptionPasswordChanged() {
  ...
  protected String getNewEncryptionPassword() {
    ...
    return PASSWORD_MASK.equals(input)
        ? encryption.decrypt(caldavAccount.getEncryptionPassword())
        : input;
  }
  ...
  private void save() {
    ...
    if (this instanceof EteSyncAccountSettingsActivity && isEmpty(encryptionPassword)) {
    ...
    if (caldavAccount == null) {
      ...
      addAccount(url, username, password, encryptionPassword);
    } else if (needsValidation()) {
      showProgressIndicator();
      updateAccount(url, username, password, encryptionPassword);
    } else if (hasChanges()) {
  ...
  protected void updateAccount(String principal) {
    ...
    if (this instanceof EteSyncAccountSettingsActivity && encryptionPasswordChanged()) {
      caldavAccount.setEncryptionPassword(encryption.encrypt(getNewEncryptionPassword()));
    }
  ...
  private boolean needsValidation() {
    return !getNewURL().equals(caldavAccount.getUrl())
        ...
        || passwordChanged()
        || (this instanceof EteSyncAccountSettingsActivity && encryptionPasswordChanged());
  }
